...
async def archived(interaction: discord.Interaction, season: int, league: str):
    ...
    if leag == TABLE_MODES[9]:
        ...
        if season != 1:
            ...
            return
    elif leag == TABLE_MODES[11]:
        await interaction.followup.send(" ⚠ No archives for Universal Leadboard.")
        return
    elif leag == TABLE_MODES[12]:
        if season < 2:
            await interaction.followup.send(
                "⚠ Sesonal Leadboard Archives does not exist for season 1 and 2."
            )
            return
    else:
        ...
        if status == SEASON_STATUS[3]:
            ...
            await interaction.followup.send(
                "❌ Error occurred processing request, please contact the admins"
            )
            return
        elif status == SEASON_STATUS[1]:
            await interaction.followup.send(
                "❌ This session is ongoing, please use !show command."
            )
            return
    ...
    try:
        if leag == TABLE_MODES[9]:
            ...
            headers, rows = await get_table_data(le, stat=str(season))
        else:
            headers, rows = await get_table_data(st)
    except Exception as e:
        print(f"error: {e}")
        await interaction.followup.send(
            content="⚠️ Failed to load data.", ephemeral=True
        )
        return

    if not rows:
        await interaction.followup.send(
            content="⚠ This table is empty.", ephemeral=True
        )
        return
    ...
    if season == 0:
        ...
        embed = discord.Embed(title=f"Sesonal Leadboard - Season {season}")
    else:
