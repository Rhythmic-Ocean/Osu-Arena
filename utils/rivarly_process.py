...
class ChallengeView(View):
    ...
    """
    def __init__(self, challenged: discord.Member, timeout: float = 86400):
    ...
    async def accept(self, interaction: discord.Interaction, button: Button):
        ...
        if interaction.user != self.challenged:
            ...
            return
        self.response = True
        await interaction.response.send_message(f"{self.challenged.mention} has accepted the challenge!")
        self.stop()
    ...
    async def decline(self, interaction: discord.Interaction, button: Button):
        ...
        if interaction.user != self.challenged:
            ...
            return
        self.response = False
        await interaction.response.send_message(f"{self.challenged.mention} has declined the challenge.")
        self.stop()

async def log_rivals(leag: str, challenger: str, challenged: str, pp: float) -> Optional[int]:
    """
    ...
    try:
        ...
        if not challenger_uname or not challenged_uname:
            logging.warning(f"Missing osu username for challenger ({challenger}) or challenged ({challenged}).")
            return None
        ...
        if challenger_pp is None or challenged_pp is None:
            logging.warning(f"Missing PP data for challenger ({challenger}) or challenged ({challenged}).")
            return None
        ...
        try:
            ...
            logging.error(f"Error inserting into sub-tables (challenger/challenged): {e}")
...
async def store_msg_id(challenge_id: int, msg_id: int) -> None:
    ...
    supabase = await create_supabase()
    try:
        await supabase.table('mesg_id').insert({
            "msg_id": msg_id, 
            "challenge_id": challenge_id
        }).execute()
    except Exception as e:
