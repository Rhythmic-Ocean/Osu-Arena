...
async def get_historical_points(stat):
    column_name = f"season_{stat}"
    historical_supabase = await create_supabase()
    try:
        response = (
            await historical_supabase.table("historical_points")
            .select(f"osu_username, points:{column_name}")
            .limit(15)
