...
class SeasonManagement(commands.Cog):
    ...
    async def season_end(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await self.log_handler.report_error("Season Ending Critical Failure", e)
            await interaction.edit_original_response(
                content="❌ **CRITICAL ERROR** during ending sequence. Check logs."
            )
    ...
    async def _step_archive_season(
        ...
        if not current:
            await interaction.followup.send(
                "❌ No ongoing season found. Command Cancelled"
            )
            return None
